            ...
            <div class="modal-body">
                <form class="form-horizontal">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <input type="text" name="nMinAddress" class="form-control" id="min_ip_update_input"
                                   placeholder="请输入最小IP">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <input type="text" name="nMaxAddress" class="form-control" id="max_ip_update_input"
                                   placeholder="请输入最大IP">
                            <span class="help-block"></span>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    function build_emps_table(result) {
        ...
        $.each(emps, function (index, item) {
            ...
            var nId = $("<td></td>").append(item.nId).attr("title", item.nId);
            var oId = $("<td></td>").append(item.oId).attr("title", item.oId);
            var address = $("<td></td>").append(item.nMinAddress + " - " + item.nMaxAddress).attr("title", item.nMinAddress + "-" + item.nMaxAddress);
...
//        window.location.href='data/gotoImport';
    ...
    function validate_add_form() {
        var min_ip_add_input = $("#min_ip_add_input").val();
        if(min_ip_add_input == null||min_ip_add_input == "" ){
            show_validate_msg("#min_ip_add_input", "error", "网络地址段最小ip不能为空");
            return false;
        }else if(!ValidateIPaddress(min_ip_add_input)){
            show_validate_msg("#min_ip_add_input", "error", "网络地址段最小ip格式不符合");
            return false;
        }else {
            show_validate_msg("#min_ip_add_input", "success", "");
        }
        var max_ip_add_input = $("#max_ip_add_input").val();
        if (max_ip_add_input == null || max_ip_add_input == "") {
            show_validate_msg("#max_ip_add_input", "error", "网络地址段最大ip不能为空");
            return false;
        } else if (!ValidateIPaddress(max_ip_add_input)) {
            show_validate_msg("#max_ip_add_input", "error", "网络地址段最大ip格式不符合");
            return false;
        } else {
            show_validate_msg("#max_ip_add_input", "success", "");
        }
        ...
        if(mask_add_input == null||mask_add_input == "" ){
            ...
            show_validate_msg("#mask_add_input", "error", "网络地址段最大ip格式不符合");
            return false;
        } else  {
 ...
 //校验更新表单数据
    function validate_update_form() {
        var min_ip_update_input = $("#min_ip_update_input").val();
        if(min_ip_update_input == null||min_ip_update_input == "" ){
            show_validate_msg("#min_ip_update_input", "error", "网络地址段最小ip不能为空");
            return false;
        }else if(!ValidateIPaddress(min_ip_update_input)){
            show_validate_msg("#min_ip_update_input", "error", "网络地址段最小ip格式不符合");
            return false;
        }else {
            show_validate_msg("#min_ip_update_input", "success", "");
        }
        var max_ip_update_input = $("#max_ip_update_input").val();
        if (max_ip_update_input == null || max_ip_update_input == "") {
            show_validate_msg("#max_ip_update_input", "error", "网络地址段最大ip不能为空");
            return false;
        } else if (!ValidateIPaddress(max_ip_update_input)) {
            show_validate_msg("#max_ip_update_input", "error", "网络地址段最大ip格式不符合");
            return false;
        } else {
            show_validate_msg("#max_ip_update_input", "success", "");
        }
...
//                alert(myData.oId);
  ...
  */
        //2、发送ajax请求保存更新的员工数据
        $.ajax({
            url: "${APP_PATH}/a/network/" + $(this).attr("edit-id"),
            type: "PUT",
...
//        alert($(this).parents("tr").find("td:eq(1)").text());
    ...
    $("#emp_delete_all_btn").click(function () {
        ...
        if (confirm("确认删除【" + ipNames + "】的数据吗？")) {
            //发送ajax请求删除
            $.ajax({
                url: "${APP_PATH}/a/network/" + del_idstr,
                type: "DELETE",
                success: function (result) {
                    alert(result.msg);
                    //回到当前页面
